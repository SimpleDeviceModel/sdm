...
To run SDM, a target system must have shared versions of the Standard \cplusplus{} library and Qt libraries installed. On Microsoft Windows, the \shellcmd{install} target will run a deployment automation helper tool (\shellcmd{dlldeptool.exe}). Qt libraries are always installed to the application directory. Standard \cplusplus{} runtime libraries are usually deployed centrally with the help of a redistributable package, unless \shellcmd{OPTION\_USE\_VCREDIST} is \shellcmd{OFF} or it's a portable installation. If you plan to distribute the produced package, ensure that you are allowed to redistribute these libraries. Notably, debug versions of Microsoft Visual \cplusplus{} Runtime are \emph{not} redistributable.
