...
SDM requires a modern version of Linux or Microsoft Windows. The current release has been tested on a 64-bit machine with the following operating systems:
...
Other OS versions supported by Qt\ 6 may work as well.
...
There is also a portable version that stores settings in the application directory and does not require installation.
...
\begin{shellcmds}
sudo apt install build-essentials qt6-base-dev \textbackslash
    qt6-base-dev-tools libqt6svg6-dev qt6-tools-dev \textbackslash
    qt6-tools-dev-tools qt6-l10n-tools \textbackslash
    qt6-translations-l10n qt6-image-formats-plugins cmake
\end{shellcmds}
...
\begin{itemize}
	\item A toolchain for \cplusplus{} development. This release has been tested with the following toolchains:
		\begin{itemize}
			\item For Microsoft Windows: Microsoft Visual Studio® 2022.
			\item For Linux: GCC 11.4, Clang 14.0.
		\end{itemize}
	\item CMake 3.22 or later;
	\item Qt® 6.x (optional, for \shellcmd{sdmconsole});
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \shellcmd{"NMake Makefiles JOM"} (same with parallel build support)
		\item \shellcmd{"Visual Studio 17 2022"} (IDE-based)
	\end{itemize}
...
\begin{enumerate}[resume]
	\item Build the source code. For Makefile-based generators, a \shellcmd{make} command must be invoked (\shellcmd{nmake} or \shellcmd{jom} for Windows)\footnote{\shellcmd{jom} is an \shellcmd{nmake} clone developed by the Qt project. Unlike \shellcmd{nmake}, it supports parallel builds. See \url{https://wiki.qt.io/Jom}.}.
	
	For Visual Studio generators, build the \shellcmd{ALL\_BUILD} target.
	
	\item To install the project, invoke \shellcmd{make install} for Makefile-based generators or build the \shellcmd{INSTALL} target for Visual Studio generators.
	
	\item Optionally, one can run the SDM test suite by invoking \shellcmd{make test} for Makefile-based generators or building the \shellcmd{RUN\_TESTS} target for Visual Studio generators.
\end{enumerate}
...
\shellcmd{OPTION\_NO\_QT} & Don't use Qt. \shellcmd{sdmconsole} won't be built. \\

\shellcmd{OPTION\_PORTABLE} & Portable installation (stores configuration in the application directory). \\

\shellcmd{OPTION\_USE\_VCREDIST} & When building the installer package, deploy the Microsoft Visual \cplusplus{} Runtime using \shellcmd{vcredist\_*.exe} (MSVC only). See also Subsection \ref{subsec:wininstaller}. \\

\shellcmd{OPTION\_VALGRIND} & Enable using \shellcmd{valgrind} by the test suite to track memory leaks (Linux only). \\

\shellcmd{OPTION\_NO\_ICONS} & Don't install desktop entries (Linux only). \\

\shellcmd{OPTION\_NO\_LINENOISE} & Don't use the linenoise line editing library for sdmhost (Linux only). \\
...
On Microsoft Windows the default value of the \shellcmd{CMAKE\_INSTALL\_PREFIX} variable is \shellcmd{C:/Program Files/Simple Device Model}. It is used only by the CMake \shellcmd{install} target and doesn't affect compilation.
...
Installer script will not be generated when \shellcmd{OPTION\_PORTABLE} is set.

\subsubsection{External DLL dependencies}

To run SDM, a target system must have shared versions of the Standard \cplusplus{} library and Qt libraries. On Microsoft Windows, the \shellcmd{install} target will run a deployment automation tool (\shellcmd{dlldeptool.exe}) that will collect the necessary DLLs from the host system. Qt libraries are always installed to the application directory. Standard \cplusplus{} runtime libraries are usually deployed centrally with the help of a redistributable package, unless \shellcmd{OPTION\_USE\_VCREDIST} is \shellcmd{OFF} or it's a portable installation. If you plan to distribute the produced package, ensure that you are allowed to redistribute these libraries. Notably, debug versions of Microsoft Visual \cplusplus{} Runtime are \emph{not} redistributable.
...
User settings are stored in \shellcmd{\%APPDATA\%\textbackslash{}Simple Device Model} on Microsoft Windows and in \shellcmd{\$HOME/.config/Simple Device Model} on Linux for non-portable builds. Portable builds store settings in the \shellcmd{.config} subdirectory of the application directory.
...
\begin{funcremarks}
	FIFO is a memory block mapped to a single register address. Calling \luaexpr{writefifo()} can be more efficient than calling \luaexpr{writereg()} multiple times depending on how the plugin is implemented.
\end{funcremarks}
...
\begin{funcremarks}
	FIFO is a memory block mapped to a single register address. Calling \luaexpr{readfifo()} can be more efficient than calling \luaexpr{readreg()} multiple times depending on how the plugin is implemented.
\end{funcremarks}
...
\begin{funcremarks}
	Calling \luaexpr{writemem()} can be more efficient than calling \luaexpr{writereg()} multiple times depending on how the plugin is implemented.
\end{funcremarks}
...
\begin{funcremarks}
	Calling \luaexpr{readmem()} can be more efficient than calling \luaexpr{readreg()} multiple times depending on how the plugin is implemented.
\end{funcremarks}
...
\expr{luadfic} is a low-level library which by design is able to circumvent most safety mechanisms. Incorrect usage may result in program crashes.
...
\begin{funcret}
	Returns a non-negative value if successful, a negative value otherwise. Note: plugins should return the number of words written if compatibility with SDM $\le$ 1.0.5 is required.
\end{funcret}

\begin{funcremarks}
	FIFO is a memory block that is mapped to a single register address. Writing to a FIFO is conceptually equivalent to performing a series of ordinary register writes, but can be more efficient for devices supporting block transactions.
\end{funcremarks}
...
\begin{funcret}
	Returns a non-negative value if successful, a negative value otherwise. Note: plugins should return the number of words read if compatibility with SDM $\le$ 1.0.5 is required.
\end{funcret}

\begin{funcremarks}
	FIFO is a memory block that is mapped to a single register address. Reading from a FIFO is conceptually equivalent to performing a series of ordinary register reads, but can be more efficient for devices supporting block transactions.
\end{funcremarks}
...
\begin{funcremarks}
	Writing to memory is conceptually equivalent to performing a series of register writes with incrementing addresses, but can be more efficient for devices supporting block transactions. Depending on address granularity, the address can be incremented by a value other than 1.
\end{funcremarks}
...
\begin{funcremarks}
	Reading from memory is conceptually equivalent to performing a series of register reads with incrementing addresses, but can be more efficient for devices supporting block transactions. Depending on address granularity, the address can be incremented by a value other than 1.
\end{funcremarks}
