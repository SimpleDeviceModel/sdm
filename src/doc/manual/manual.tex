...
\begin{itemize}
	...
	\item Qt® 5.x, x $\ge$ 2 (optional, for \shellcmd{sdmconsole});
...
\shellcmd{EXTRA\_PLUGINS\_DIR} & Path to an out of tree plugins source directory to be build alongside SDM. The directory must contain CMakeLists.txt. Optional.\\
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{lL}
		...
		\luaexpr{lfs} & LuaFileSystem: access directory structure and file attributes \\
...
\luaexpr{lfs} is a third-party extension module developed by the Kepler Project\footnote{\url{https://keplerproject.github.io/luafilesystem/}}. It comes with its own documentation and will not be covered in this manual. \luaexpr{luadfic}, \luaexpr{luaipsockets} and \luaexpr{luart} are developed as parts of the SDM project, but can be also used with a standalone Lua interpreter.
...
\begin{funcparams}
	...
	\funcparam{packets} (\luatype{integer}, optional): suggested minimum number of packets to deliver consecutively (per stream), \cexpr{0} for default
	\funcparam{df} (\luatype{integer}, optional): decimation factor (only one of each \luaexpr{df} packets will be delivered)
\end{funcparams}
...
SDM SDK includes an example, \shellcmd{testplugin}, based on \shellcmd{pluginprovider}. It is more advanced than \shellcmd{simpleplugin} and is used by the SDM internal test suite. To build \shellcmd{testplugin}, a modern compiler supporting \cplusplus{}11 standard is required (\shellcmd{pluginprovider} itself doesn't use \cplusplus{}11 features).
...
\shellcmd{sdm::ipsockets} and \shellcmd{sdm::uart} are the \cplusplus{} libraries on which the \shellcmd{luaipsockets} and \shellcmd{luart} bindings are based (see Chapter \ref{ch:scripting}). These libraries use the RAII principle to control object lifetime. Error handling is based on \cplusplus{} exceptions. To use these libraries, include \shellcmd{"ipsocket.h"} or \shellcmd{"uart.h"}, respectively, and link against the corresponding library files.
...
\begin{funcparams}
	\funcparam{h}: channel handle
	\funcparam{addr}: register address
	\funcparam{status}: pointer to a variable to receive operation error status, can be \cexpr{NULL}
\end{funcparams}

\begin{funcret}
	If successful, returns a value read from the register. Otherwise \cexpr{*status} is set to a non-zero value (if \cexpr{status} is not \cexpr{NULL}) and the return value is undefined.
\end{funcret}
...
\begin{funcparams}
	...
	\funcparam{df}: decimation factor (only 1 of each \cexpr{df} packets will be delivered)
\end{funcparams}
